...
serve(async (req) => {
  ...
  try {
    ...
    if (existingProfile?.id) {
      const accessStatus = String(patient.acesso_status || "").trim().toLowerCase();
      const alreadyInvited = accessStatus === "convite_enviado" || Boolean(patient.convite_enviado_em || patient.ultimo_convite_em);
      const alreadyActive = accessStatus === "ativo" || patient.acesso_liberado === true || Boolean(patient.acesso_liberado_em);

      let authUser: any = null;
      try {
        const { data: authUserData, error: authUserError } = await adminClient.auth.admin.getUserById(existingProfile.id);
        if (authUserError) {
          console.warn("[invite-patient-access] Não foi possível verificar usuário Auth existente:", authUserError.message);
        }
        authUser = authUserData?.user || null;
      } catch (authCheckError) {
        console.warn("[invite-patient-access] Falha ao consultar usuário Auth existente:", authCheckError);
      }
      ...
      if (!alreadyActive && (alreadyInvited || !acceptedInvite)) {
        const { error: updatePendingInviteError } = await adminClient
          .from("pacientes")
          .update({
            perfil_id: existingProfile.id,
            acesso_status: "convite_enviado",
            convite_enviado_em: patient.convite_enviado_em || now,
            ultimo_convite_em: now,
          })
          .eq("id", patientId);

        if (updatePendingInviteError) throw updatePendingInviteError;

        return json({
          success: true,
          status: "invite_pending",
          message: "Convite já enviado. O acesso ficará ativo quando o paciente concluir o cadastro.",
          patientId,
