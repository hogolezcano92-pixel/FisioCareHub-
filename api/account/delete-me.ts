...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    if (requestedUserId && requestedUserId !== user.id) {
      return response.status(403).json({ success: false, error: 'Você só pode apagar a sua própria conta.' });
    }

    const report = await deleteUserDataCascade(supabaseAdmin, user.id);
    ...
    return response.status(200).json({
      success: true,
      message: 'Conta e dados vinculados apagados permanentemente.',
      deletedUserId: user.id,
      report,
    });
  } catch (error: any) {
    console.error('[delete-me] Fatal error:', error);
    return response.status(500).json({
      success: false,
      error: error?.message || 'Erro fatal ao apagar conta.',
    });
  }
}
