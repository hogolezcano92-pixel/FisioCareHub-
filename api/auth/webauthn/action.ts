...
/**
 * O erro "The string did not match the expected pattern" no Safari/iOS costuma
 * aparecer quando o rpID enviado ao navegador não bate exatamente com o domínio
 * atual ou vem com protocolo/caminho.
 *
 * Por isso, para WebAuthn, usamos o host REAL da requisição da página atual:
 * - fisio-care...vercel.app
 * - fisiocarehub.company
 * - localhost
 *
 * Nunca usamos URL completa como rpID.
 */
function getRequestWebAuthnContext(req: VercelRequest) {
  const fallback = getWebAuthnConfig(req);
  ...
  const rawHost = Array.isArray(forwardedHostHeader)
    ...
    : forwardedHostHeader || hostHeader || fallback.rpID;

  const rawProtoHeader = req.headers['x-forwarded-proto'];
  const protocol = String(
    Array.isArray(rawProtoHeader) ? rawProtoHeader[0] : rawProtoHeader || 'https'
  )
    .split(',')[0]
    .trim();

  const hostWithoutProtocol = String(rawHost || '')
    .split(',')[0]
    .replace(/^https?:\/\//i, '')
    .split('/')[0]
    .trim()
  ...
  const isLocalhost = hostWithoutProtocol.startsWith('localhost');
  const rpID = isLocalhost ? 'localhost' : hostWithoutProtocol.split(':')[0];

  if (!rpID) {
    throw new Error('Não foi possível identificar o domínio para biometria.');
  }

  return {
    rpName: fallback.rpName || 'FisioCareHub',
    rpID,
    origin: `${protocol}://${hostWithoutProtocol}`,
  };
}
...
async function registrationOptions(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { rpName, rpID } = getRequestWebAuthnContext(req);
    ...
    const options = await generateRegistrationOptions({
      ...
      // Mantém vazio para evitar credential_id antigo/fora de base64url quebrar o Safari.
      excludeCredentials: [],
     ...
     * a versão atual do @simplewebauthn/server exige rpID para gerar as opções,
     * mas enviar rp.id ao navegador pode causar:
     * "The string did not match the expected pattern"
     * quando há proxy/domínio com www/Vercel.
     *
     * Ao remover rp.id da resposta, o navegador usa automaticamente o domínio
     * atual da página, que é exatamente o comportamento mais seguro para iOS.
     */
    ...
    delete (browserOptions.rp as { id?: string }).id;

    return res.status(200).json(browserOptions);
  } catch (error) {
...
async function register(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { rpID, origin } = getRequestWebAuthnContext(req);
...
async function loginOptions(req: VercelRequest, res: VercelResponse) {
  // O fluxo principal usa POST com o e-mail no body.
  // Também aceitamos GET como fallback seguro para evitar erro 405 em ambientes
  // onde uma versão antiga/cacheada do frontend ou rota ainda tente consultar por query string.
  if (assertMethod(req, res, ['POST', 'GET']) !== true) return;

  try {
    const body = getRequestBody<LoginOptionsBody>(req);
    const queryEmail = Array.isArray(req.query.email) ? req.query.email[0] : req.query.email;
    const email = req.method === 'GET' ? queryEmail : body.email;
    ...
    const { rpID } = getRequestWebAuthnContext(req);
    ...
    /**
     * Mesmo motivo do cadastro: deixa o navegador usar o domínio atual
     * em vez de forçar rpId na resposta.
     */
    const browserOptions = { ...options };
    delete (browserOptions as { rpId?: string }).rpId;
