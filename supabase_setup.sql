...
CREATE TABLE IF NOT EXISTS public.mensagens (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    remetente UUID REFERENCES public.perfis(id) ON DELETE CASCADE NOT NULL,
    destinatario UUID REFERENCES public.perfis(id) ON DELETE CASCADE NOT NULL,
    mensagem TEXT NOT NULL,
    lida BOOLEAN DEFAULT false,
...
CREATE POLICY "Usuários veem suas mensagens" ON public.mensagens FOR SELECT USING (auth.uid() = remetente OR auth.uid() = destinatario);
CREATE POLICY "Usuários enviam mensagens" ON public.mensagens FOR INSERT WITH CHECK (auth.uid() = remetente);
