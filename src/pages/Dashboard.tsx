    ...
    <div className="min-h-screen pt-4 md:pt-8 pb-12 bg-[#0B1120] relative overflow-hidden transition-colors duration-500">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 md:space-y-10 relative z-10">
        ...
        {isPhysio ? (
          ...
          <header className="relative overflow-hidden rounded-[2rem] border border-violet-200/70 bg-white/90 p-5 sm:p-6 md:p-7 shadow-[0_24px_80px_rgba(79,70,229,0.16)] backdrop-blur-3xl dark:border-white/10 dark:bg-white/5 dark:shadow-blue-900/20">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_0%,rgba(14,165,233,0.14),transparent_34%),radial-gradient(circle_at_92%_12%,rgba(168,85,247,0.14),transparent_30%)] pointer-events-none" />
            <div className="absolute right-5 top-4 hidden h-40 w-40 rounded-full bg-violet-500/10 blur-3xl md:block" />
            <div className="absolute right-8 top-8 hidden md:block text-violet-300/60">
              <div className="relative h-24 w-24 rounded-full border border-violet-200/70 bg-white/50 shadow-inner dark:bg-white/5">
                <Activity className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-violet-400" size={30} />
              </div>
            </div>
            <div className="absolute right-16 top-12 hidden h-52 w-72 rotate-[-12deg] rounded-full border border-sky-200/40 md:block" />
            <div className="absolute right-20 top-20 hidden h-44 w-64 rotate-[-12deg] rounded-full border border-violet-200/40 md:block" />

            <div className="relative z-10 flex flex-col gap-5">
              <div className="flex items-start gap-4 sm:gap-5">
                {!profile ? (
                  <div className="h-20 w-20 rounded-full border-4 border-white bg-slate-100 shadow-xl animate-pulse dark:border-white/10 dark:bg-slate-800" />
                ) : (
                  <div className="relative shrink-0">
                    <div className="absolute -inset-2 rounded-full bg-gradient-to-tr from-sky-400 via-indigo-400 to-violet-500 opacity-30 blur-md" />
                    <div className="relative rounded-full bg-gradient-to-tr from-sky-400 via-indigo-400 to-violet-500 p-[3px] shadow-2xl shadow-indigo-500/20">
                      <img
                        src={profile.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profile.id}`}
                        alt={profile.nome_completo}
                        className="h-20 w-20 rounded-full border-4 border-white object-cover dark:border-slate-950"
                      />
                    </div>
                    <span className="absolute bottom-1 right-0 h-5 w-5 rounded-full border-[3px] border-white bg-emerald-400 shadow-lg shadow-emerald-500/30 dark:border-slate-950" />
                  </div>
                )}
              </div>
              ...
              <div className="max-w-4xl">
                ...
                {!profile ? (
                  <span className="mt-2 block text-3xl font-black text-slate-300 animate-pulse">Conectando...</span>
                ) : (
                  <div className="mt-2 flex flex-wrap items-center gap-3">
                    <span className="text-4xl font-black uppercase leading-none tracking-tight bg-gradient-to-r from-sky-500 via-indigo-500 to-violet-600 bg-clip-text text-transparent sm:text-5xl md:text-6xl">
                      {profile.nome_completo}
                    </span>
                    <span className="inline-flex items-center rounded-full border border-violet-300/80 bg-violet-100/80 px-4 py-1.5 text-sm font-black text-violet-700 shadow-sm dark:border-violet-400/30 dark:bg-violet-500/10 dark:text-violet-200 sm:text-base">
                      Paciente
                    </span>
                  </div>
                )}

                <p className="mt-4 max-w-3xl text-sm font-bold leading-relaxed text-slate-600 dark:text-slate-300 sm:text-base md:text-lg">
                  Bem-vindo à sua área de acompanhamento. Continue sua jornada de recuperação com suporte inteligente, triagens e evolução clínica.
                </p>
              </div>

              <div className="inline-flex w-fit items-center gap-2 rounded-full border border-violet-200 bg-white/75 px-4 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-sky-600 shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-sky-300 sm:text-xs">
                <Sparkles size={15} className="text-sky-500" />
                Sua jornada de saúde
              </div>

              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div className="flex items-center justify-between rounded-2xl border border-sky-100 bg-white/80 p-3 shadow-sm dark:border-white/10 dark:bg-white/5">
                  <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-full bg-sky-100 text-sky-600 dark:bg-sky-500/10 dark:text-sky-300">
                      <Activity size={21} />
                    </div>
                    <div>
                      <p className="text-xs font-black text-slate-900 dark:text-white">Dor</p>
                      <p className="text-sm font-extrabold text-sky-600 dark:text-sky-300">Acompanhe</p>
                    </div>
                  </div>
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" />
                </div>

                <div className="flex items-center justify-between rounded-2xl border border-violet-100 bg-white/80 p-3 shadow-sm dark:border-white/10 dark:bg-white/5">
                  <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-full bg-violet-100 text-violet-600 dark:bg-violet-500/10 dark:text-violet-300">
                      <Check size={21} />
                    </div>
                    <div>
                      <p className="text-xs font-black text-slate-900 dark:text-white">Exercícios</p>
                      <p className="text-sm font-extrabold text-violet-600 dark:text-violet-300">{stats.records} registro{stats.records === 1 ? '' : 's'}</p>
                    </div>
                  </div>
                </div>

                <div className="flex items-center justify-between rounded-2xl border border-blue-100 bg-white/80 p-3 shadow-sm dark:border-white/10 dark:bg-white/5">
                  <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-full bg-blue-100 text-blue-600 dark:bg-blue-500/10 dark:text-blue-300">
                      <Calendar size={21} />
                    </div>
                    <div>
                      <p className="text-xs font-black text-slate-900 dark:text-white">Consulta</p>
                      <p className="text-sm font-extrabold text-blue-600 dark:text-blue-300">{stats.appointments > 0 ? 'Em dia' : 'Agende'}</p>
                    </div>
                  </div>
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" />
                </div>
              ...
              <button
                ...
                className="group flex w-full items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-sky-500 via-indigo-500 to-violet-600 px-6 py-4 text-base font-black text-white shadow-xl shadow-indigo-500/25 transition-all hover:-translate-y-0.5 hover:shadow-2xl hover:shadow-indigo-500/35 active:translate-y-0 sm:text-lg"
              >
                <Plus size={26} className="stroke-[3px] transition-transform group-hover:rotate-90" />
                Nova Triagem
              </button>
            </div>
          </header>
