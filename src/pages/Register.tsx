...
export default function Register() {
  ...
  const handleGoogleLogin = async (selectedRole: 'paciente' | 'fisioterapeuta' = role) => {
    setLoading(true);
    setError('');

    try {
      // O cadastro com Google precisa carregar explicitamente o tipo de usuário.
      // Sem isso, o AuthContext pode criar um perfil básico como paciente por padrão.
      localStorage.setItem('pending_role', selectedRole);
      localStorage.setItem('oauth_registration_flow', '1');
        ...
        <div className="bg-card/50 backdrop-blur-2xl p-8 sm:p-12 rounded-[3.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] border border-white/10">
          ...
          <form onSubmit={handleRegister} className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <button
                type="button"
                onClick={() => handleGoogleLogin('paciente')}
                disabled={loading}
              ...
              >
                ...
                Cadastrar com Google como Paciente
              </button>

              <button
                type="button"
                onClick={() => handleGoogleLogin('fisioterapeuta')}
                disabled={loading}
                className="w-full py-4 bg-blue-600/15 border border-blue-400/30 text-white rounded-2xl font-bold text-sm hover:bg-blue-600/25 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
              >
                <Stethoscope size={20} className="text-blue-300" />
                Cadastrar com Google como Fisioterapeuta
              </button>

              <p className="text-center text-xs font-semibold text-slate-500 leading-relaxed">
                Escolha o tipo correto antes de continuar com Google. Isso evita criar uma conta de paciente por engano.
              </p>
            </div>
          </form>
