...
const parseDateValueToLocalDate = (dateValue?: string | null) => {
...
const normalizeStoredDateValue = (dateValue?: string | null) => {
  if (!dateValue) return '';
...
const getAppointmentDateValue = (appointment: any) => {
  // A coluna `data` é a data oficial escolhida na agenda.
  // Quando ela vem como timestamp/date, usamos apenas YYYY-MM-DD para evitar voltar 1 dia por fuso UTC.
  const officialDate = normalizeStoredDateValue(appointment?.data);
  if (officialDate) return officialDate;

  if (appointment?.data_servico) {
    const parsed = new Date(appointment.data_servico);
    if (!Number.isNaN(parsed.getTime())) return toLocalDateInputValue(parsed);
  }

  return '';
};

const formatDateValueBR = (dateValue?: string | null) => {
  const localDate = parseDateValueToLocalDate(normalizeStoredDateValue(dateValue));
  if (!localDate) return '--/--/----';
  return localDate.toLocaleDateString('pt-BR');
};

const formatDateValueMonthShort = (dateValue?: string | null) => {
  const localDate = parseDateValueToLocalDate(normalizeStoredDateValue(dateValue));
  if (!localDate) return '--';
  return localDate.toLocaleDateString('pt-BR', { month: 'short' });
};

const formatDateValueDay = (dateValue?: string | null) => {
  const localDate = parseDateValueToLocalDate(normalizeStoredDateValue(dateValue));
  if (!localDate) return '--';
  return String(localDate.getDate());
};

const formatDateValueWeekday = (dateValue?: string | null) => {
  const localDate = parseDateValueToLocalDate(normalizeStoredDateValue(dateValue));
  if (!localDate) return '';
  return localDate.toLocaleDateString('pt-BR', { weekday: 'long' });
};

const getAppointmentDateLabel = (appointment: any) => formatDateValueBR(getAppointmentDateValue(appointment));

const getAppointmentTimeLabel = (appointment: any) => {
...
export default function Agenda() {
    ...
    const checkUrlParams = async () => {
      ...
      if (appointmentId) {
        ...
        if (data && !fetchError) {
          ...
          const appointmentDate = getAppointmentDateValue(data);
    ...
    try {
      ...
      if (supabaseError) {
        ...
        if (fallbackData && fallbackData.length > 0) {
          const patientIds = [...new Set(fallbackData.map(a => a.paciente_id).filter(Boolean))];
          ...
          appointmentsData = fallbackData.map(a => ({
            ...a,
      ...
      const normalizedAppointments = appointmentsData.map((appointment) => ({
        ...appointment,
        _dateValue: getAppointmentDateValue(appointment),
        _dateLabel: getAppointmentDateLabel(appointment),
        _timeLabel: getAppointmentTimeLabel(appointment)
      }));

      const filteredAppointments = view === 'daily'
        ? normalizedAppointments.filter((appointment) => appointment._dateValue === selectedDate)
        : normalizedAppointments;

      filteredAppointments.sort((a, b) => {
        const dateCompare = String(a._dateValue || '').localeCompare(String(b._dateValue || ''));
        if (dateCompare !== 0) return view === 'daily' ? dateCompare : -dateCompare;
        return String(a.hora || '').localeCompare(String(b.hora || ''));
      });
      
      console.log('Agendamentos encontrados:', filteredAppointments.length);
        ...
        if (patientProfile && profile) {
          sendAppointmentConfirmation(
            ...
            profile.email,
            {
              ...
              date: formatDateValueBR(formData.data),
              time: formData.hora,
        ...
        if (patientEmail && profile) {
          ...
          if (status === 'confirmado') {
            sendAppointmentStatusEmail(
              ...
              'confirmado',
              {
                date: getAppointmentDateLabel(app),
  ...
  const changeDate = (days: number) => {
    const date = parseDateValueToLocalDate(selectedDate) || new Date();
    date.setDate(date.getDate() + days);
    setSelectedDate(toLocalDateInputValue(date));
  };
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl p-2.5 rounded-2xl border border-white/10 shadow-2xl flex items-center justify-between w-full">
          ...
          <div className="flex flex-col items-center">
            <span className="text-[8px] font-bold text-sky-400 uppercase tracking-widest mb-0.5">
              {formatDateValueWeekday(selectedDate)}
            </span>
            <input
              type="date"
              value={selectedDate}
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="flex flex-col items-center justify-center w-12 h-12 bg-white/5 rounded-xl text-white group-hover:bg-sky-500/10 transition-colors border border-white/5">
                    {view === 'all' ? (
                      <>
                        <span className="text-[7px] font-black text-sky-400 uppercase">{formatDateValueMonthShort(app._dateValue || app.data)}</span>
                        <span className="text-sm font-black leading-none">{formatDateValueDay(app._dateValue || app.data)}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="p-3.5 bg-white/5 rounded-2xl border border-white/10">
                    ...
                    <div className="flex items-center gap-2 text-white font-black text-xs">
                      ...
                      {selectedAppointment._dateLabel || getAppointmentDateLabel(selectedAppointment)}
                    </div>
                    <div className="flex items-center gap-2 text-white font-black text-xs mt-1.5">
                      <Clock size={14} className="text-sky-400" />
                      {selectedAppointment._timeLabel || getAppointmentTimeLabel(selectedAppointment)}
                    </div>
                  </div>
                  <div className="p-3.5 bg-white/5 rounded-2xl border border-white/10">
