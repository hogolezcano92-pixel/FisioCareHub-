...
import { toast } from 'sonner'; // ✅ Corrigido para a biblioteca que você tem no projeto
import { FileText, Download } from 'lucide-react';

// Tipagem simples para o componente
interface PhysioEvaluationFormProps {
  patient?: any;
  formData: any;
}

const PhysioEvaluationForm: React.FC<PhysioEvaluationFormProps> = ({ patient, formData }) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      sections.forEach((section) => {
        autoTable(doc, {
          startY: y,
          head: [[{ content: section.title, colSpan: 2, styles: { fillColor: [0, 102, 204], halign: 'center' } }]],
          body: section.data,
          theme: 'striped',
        ...
        // Atualiza o Y para a próxima seção baseado na última tabela gerada
        y = (doc as any).lastAutoTable.finalY + 10;
        
        // Se o Y chegar perto do fim da página, cria nova página
        if (y > 270) {
            doc.addPage();
            y = 20;
        }
...
export default PhysioEvaluationForm;
