...
function Section({ title, children, icon: Icon = ClipboardList }: any) {
  return (
    <section className="rounded-[2rem] border border-slate-200/90 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.07)] dark:border-white/10 dark:bg-slate-900/60 dark:shadow-none backdrop-blur-xl overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto pb-24 space-y-7">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/physio/evaluations')} className="p-3 rounded-2xl bg-white/5 text-slate-700 dark:text-slate-400 hover:text-slate-900 dark:text-white border border-white/10"><ArrowLeft size={20}/></button>
            <div>
              ...
              <p className="text-slate-700 dark:text-slate-400 text-sm mt-1">Ficha estruturada por área, com CIF e PDF profissional.</p>
            </div>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {TYPES.map(item => { const Icon=item.icon; return <button key={item.id} onClick={() => {setType(item.id);setStep(1)}} className="text-left rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_8px_24px_rgba(15,23,42,0.07)] hover:border-sky-300 hover:bg-sky-50/70 hover:shadow-[0_12px_32px_rgba(14,165,233,0.10)] dark:border-white/10 dark:bg-slate-900/60 dark:hover:border-sky-500/40 dark:hover:bg-slate-900 dark:shadow-none transition-all group">
                ...
                <h2 className="text-lg font-black text-slate-900 dark:text-white">{item.title}</h2><p className="text-sm text-slate-700 dark:text-slate-400 mt-2 leading-relaxed">{item.description}</p>
              </button>})}
            </div>
          </div>
        )}
            ...
            <div className="space-y-5">
              <div className="grid md:grid-cols-[1fr_auto] gap-3">
                <div className="relative">
                  <Search className="absolute left-4 top-3.5 text-slate-700 dark:text-slate-500" size={18}/>
        ...
        {type && step === 3 && (
          <Section title="Testes e escalas" icon={Activity}>
            ...
            <div className="space-y-3">
              ...
              {!tests.length && <div className="py-12 text-center text-slate-700 dark:text-slate-500 text-sm">Nenhum teste registrado ainda.</div>}
            </div>
        ...
        {type && step === 4 && (
          <Section title="CIF — Classificação Internacional de Funcionalidade" icon={ShieldCheck}>
            ...
            <div className="space-y-4">
              ...
              {!cif.length && <div className="py-12 text-center text-slate-700 dark:text-slate-500 text-sm">Adicione os domínios funcionais relevantes para este paciente.</div>}
            </div>
        ...
        {type && step === 5 && (
          <div className="space-y-5">
            ...
            <div className="rounded-[2rem] border border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/5 p-6 flex flex-col md:flex-row gap-5 items-start md:items-center justify-between">
              <div><div className="flex items-center gap-2 text-emerald-400 font-black"><CheckCircle2 size={20}/> Ficha pronta para finalização</div><p className="text-sm text-slate-700 dark:text-slate-400 mt-1">Salve a avaliação para vinculá-la ao histórico do paciente e gerar o PDF.</p></div>
            ...
            {integrityHash && <div className="text-[11px] text-slate-700 dark:text-slate-500 flex items-center gap-2"><ShieldCheck size={14}/> Registro protegido por integridade documental.</div>}
          </div>
        )}
      </div>
    </ProGuard>
