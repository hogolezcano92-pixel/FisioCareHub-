...
function Field({ label, value, onChange, placeholder = '', type = 'text', min, max }: any) {
  return (
    <label className="block space-y-1.5">
      <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-500">{label}</span>
      {type === 'textarea' ? (
        <textarea value={value ?? ''} onChange={e => onChange(e.target.value)} placeholder={placeholder}
          className="w-full min-h-24 rounded-2xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5 px-4 py-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-600 dark:text-slate-400 dark:placeholder:text-slate-600 outline-none focus:border-sky-500/60 focus:ring-2 focus:ring-sky-500/10 resize-y" />
      ) : (
