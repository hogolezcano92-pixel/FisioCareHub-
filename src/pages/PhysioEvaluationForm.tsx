...
function Section({ title, children, icon: Icon = ClipboardList }: any) {
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.06)] dark:border-white/10 dark:bg-slate-900/60 dark:shadow-none backdrop-blur-xl overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 bg-slate-50/50 dark:border-white/10 dark:bg-transparent flex items-center gap-3">
        <div className="p-2 rounded-xl bg-sky-500/10 text-sky-500 dark:text-sky-400"><Icon size={18} /></div>
...
export default function PhysioEvaluationWorkspace() {
  ...
  const generatePdf = async () => {
    try {
      ...
      doc.text(`Documento clínico gerado pelo FisioCareHub (CREFITO-3 461983-F). Os dados registrados são de responsabilidade do profissional.`, 15, 286);
      ...
      <div className="max-w-7xl mx-auto pb-24 space-y-7">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-[0.25em] font-black text-sky-500 dark:text-sky-400">Prontuário clínico</p>
          ...
          <div className="flex gap-2">
            {savedId && <button onClick={generatePdf} className="px-4 py-3 rounded-2xl border border-slate-300 bg-white dark:border-white/10 dark:bg-white/5 text-slate-900 dark:text-white font-black text-xs flex items-center gap-2 hover:bg-slate-50"><FileDown size={17}/> PDF premium</button>}
            <button disabled={saving || !!integrityHash} onClick={save} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-sky-500 to-blue-600 text-white dark:text-white font-black text-xs flex items-center gap-2 disabled:opacity-50 hover:shadow-lg transition-all"><Save size={17}/>{saving?'Salvando...':'Salvar avaliação'}</button>
          </div>
        </header>

        {!type && (
          <div className="space-y-5">
            <div className="flex items-center gap-2 text-slate-900 dark:text-slate-300"><Stethoscope size={20} className="text-sky-500 dark:text-sky-400"/><span className="font-black">1. Escolha a área da avaliação</span></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {TYPES.map(item => { const Icon=item.icon; return <button key={item.id} onClick={() => {setType(item.id);setStep(1)}} className="text-left rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_6px_22px_rgba(15,23,42,0.06)] hover:-translate-y-0.5 hover:border-sky-300 hover:bg-sky-50 hover:shadow-[0_12px_28px_rgba(14,165,233,0.10)] dark:border-white/10 dark:bg-slate-900/60 dark:hover:border-sky-500/40 dark:hover:bg-slate-900 transition-all group">
                <div className="w-12 h-12 rounded-2xl bg-sky-500/10 text-sky-600 dark:text-sky-400 flex items-center justify-center mb-5 group-hover:scale-105 transition-transform"><Icon size={25}/></div>
        ...
        {type && step >= 1 && (
          <div className="flex flex-wrap gap-2">
            {['Paciente','Anamnese','Exame físico','Testes','CIF','Plano'].map((x,i)=><button key={x} onClick={()=>setStep(i+1)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-colors ${step===i+1?'bg-sky-500 text-white border-sky-500 shadow-md':'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200 dark:bg-white/5 dark:text-slate-400 dark:border-white/10 dark:hover:bg-white/10'}`}>{i+1}. {x}</button>)}
          </div>
        )}
            ...
            <div className="space-y-5">
              <div className="grid md:grid-cols-[1fr_auto] gap-3">
                <div className="relative">
                  <Search className="absolute left-4 top-3.5 text-slate-500 dark:text-slate-500" size={18}/>
                  <input value={patientSearch} onChange={e=>{setPatientSearch(e.target.value); if (patientId) setPatientId('')}} placeholder="Pesquisar paciente cadastrado..." className="w-full h-12 rounded-2xl border border-slate-300 bg-white dark:border-white/10 dark:bg-white/5 pl-11 pr-4 text-slate-900 dark:text-white outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/10"/>
                  {patientSearch && !patientId && <div className="absolute z-30 mt-2 w-full rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-slate-950 shadow-2xl overflow-hidden">{filteredPatients.map(p=><button key={p.id} onClick={()=>{setPatientId(p.id);setPatientSearch(p.nome_completo)}} className="w-full text-left px-4 py-3 hover:bg-slate-100 dark:hover:bg-white/5 text-slate-900 dark:text-white text-sm flex justify-between"><span>{p.nome_completo}</span><span className="text-slate-500 dark:text-slate-500">{ageFromDate(p.data_nascimento) ? ageFromDate(p.data_nascimento)+' anos' : 'Idade não informada'}</span></button>)}</div>}
                </div>
              ...
              {showCreatePatient && <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 shadow-sm dark:border-sky-500/20 dark:bg-sky-500/5 dark:shadow-none p-5 space-y-4">
                ...
                <div className="flex justify-end"><button type="button" disabled={creatingPatient || !newPatient.nome_completo.trim()} onClick={createPatient} className="px-5 py-3 rounded-2xl bg-emerald-500 text-white font-black text-sm disabled:opacity-50 flex items-center gap-2 hover:bg-emerald-600">{creatingPatient ? <Loader2 size={17} className="animate-spin"/> : <Plus size={17}/>} Cadastrar e vincular</button></div>
              </div>}
              {patient && <div className="rounded-2xl bg-sky-50 border border-sky-200 dark:bg-sky-500/5 dark:border-sky-500/20 p-5 flex items-center justify-between"><div><p className="text-xs font-black text-sky-600 dark:text-sky-400 uppercase tracking-widest">Paciente selecionado</p><h3 className="text-xl font-black text-slate-900 dark:text-white mt-1">{patient.nome_completo}</h3><p className="text-sm text-slate-700 dark:text-slate-400">{ageFromDate(patient.data_nascimento) ? `${ageFromDate(patient.data_nascimento)} anos` : 'Idade não informada'}{patient.diagnostico ? ` • ${patient.diagnostico}` : ''}</p></div><CheckCircle2 className="text-emerald-500 dark:text-emerald-400"/></div>}
              <div className="flex justify-end"><button disabled={!patientId} onClick={()=>setStep(2)} className="px-6 py-3 rounded-2xl bg-sky-500 text-white font-black text-sm disabled:opacity-40 hover:bg-sky-600">Continuar</button></div>
            </div>
          </Section>
        )}
        ...
        {type && step === 2 && (
          <div className="space-y-5">
            ...
            <Section title="Exame físico" icon={Activity}>
              ...
              <div className="flex justify-end mt-5"><button onClick={()=>setStep(3)} className="px-6 py-3 rounded-2xl bg-sky-500 text-white hover:bg-sky-600 font-black">Próximo: testes e escalas</button></div>
            </Section>
          </div>
        )}
          ...
          <Section title="CIF — Classificação Internacional de Funcionalidade" icon={ShieldCheck}>
            ...
            <div className="space-y-4">
              {cif.map((x,i)=><div key={i} className="grid md:grid-cols-[0.55fr_1.6fr_1fr_0.55fr_1.5fr_auto] gap-3 p-4 rounded-2xl bg-white border border-slate-200 dark:bg-white/5 dark:border-white/5">
                ...
                <select value={x.qualifier} onChange={e=>setCif(a=>a.map((v,j)=>j===i?{...v,qualifier:e.target.value}:v))} className={inputCompact}><option value="0">0 — nenhum</option><option value="1">1 — leve</option><option value="2">2 — moderado</option><option value="3">3 — grave</option><option value="4">4 — completo</option><option value="8">8 — não especificado</option><option value="9">9 — não aplicável</option></select>
        ...
        {type && step === 5 && (
          <div className="space-y-5">
            ...
            <div className="rounded-[2rem] border border-emerald-300 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/5 p-6 flex flex-col md:flex-row gap-5 items-start md:items-center justify-between">
