...
export default function Login() {
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      ...
      if (passkeyError) {
        if (passkeyError.message.includes('not supported')) {
          throw new Error('Seu navegador ou dispositivo não suporta biometria/passkeys.');
        }
        throw passkeyError;
      }

      const { toast } = await import('sonner');
      toast.success('Autenticação biométrica realizada!');
      navigate('/dashboard');
    } catch (err: any) {
      console.error("Erro no login biométrico:", err);
      setError('Erro na biometria: ' + (err.message || 'Falha ao autenticar. Verifique se você já cadastrou sua biometria no perfil.'));
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              disabled={loading}
              className="py-4 bg-white border-2 border-slate-100 text-slate-700 rounded-2xl font-bold text-base hover:bg-slate-50 hover:border-slate-200 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
            >
            ...
            <button
              ...
              className="py-4 bg-white border-2 border-slate-100 text-slate-700 rounded-2xl font-bold text-base hover:bg-slate-50 hover:border-slate-200 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
            >
              <div className="w-5 h-5 flex items-center justify-center bg-blue-600 rounded-md">
