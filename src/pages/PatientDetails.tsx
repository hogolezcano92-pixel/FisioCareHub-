...
export default function PatientDetails() {
    ...
    try {
      ...
      if (patientData?.email) {
        const { data: profileByEmail } = await supabase
          ...
          .eq('email', String(patientData.email).trim().toLowerCase())
          .maybeSingle();

        linkedProfile = profileByEmail || null;
      }
      ...
      setPatient({
        ...
        acesso_ativo: Boolean(
          patientData?.acesso_status === 'ativo' ||
          patientData?.status_acesso === 'ativo' ||
          linkedProfile?.id
        ),
      });
  ...
  const handleInvitePatientAccess = async () => {
    if (!id || invitingAccess) return;

    if (!patient?.email) {
      toast.error('Paciente sem e-mail', {
        description: 'Cadastre um e-mail para enviar o convite de acesso.',
      });
      return;
    }
    ...
    try {
      ...
      if (data?.status === 'already_active' || data?.status === 'linked_existing_account') {
        toast.success('Paciente já possui conta ativa', {
          description: 'O paciente foi vinculado ao prontuário e poderá acessar os dados disponíveis.',
        });
      } else {
        toast.success('Convite de acesso enviado!', {
          description: `Enviamos um convite para ${patient.email}.`,
        });
      }
      ...
      toast.error('Erro ao enviar convite', {
        description: err?.message || 'Não foi possível liberar o acesso agora.',
      });
    } finally {
      ...
      <header className="bg-slate-900/50 backdrop-blur-xl p-8 rounded-[3rem] border border-white/10 shadow-2xl flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="flex gap-2">
          <ProGuard variant="inline">
            {patient.acesso_ativo || patient.acesso_status === 'ativo' || patient.status_acesso === 'ativo' ? (
              <div className="px-6 py-2 bg-emerald-600/20 text-emerald-300 rounded-2xl border border-emerald-600/20 font-black text-xs uppercase tracking-widest flex items-center gap-2">
                <CheckCircle2 size={18} /> Acesso ativo
              </div>
            ) : patient.acesso_status === 'convite_enviado' || patient.status_acesso === 'convite_enviado' ? (
              <button
                onClick={handleInvitePatientAccess}
                disabled={invitingAccess || !patient.email}
                className="px-6 py-2 bg-amber-600/20 text-amber-300 rounded-2xl hover:bg-amber-600/30 transition-all border border-amber-600/20 font-black text-xs uppercase tracking-widest flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {invitingAccess ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}
                Reenviar convite
              </button>
            ) : (
              <button
                onClick={handleInvitePatientAccess}
                disabled={invitingAccess || !patient.email}
                className="px-6 py-2 bg-emerald-600/20 text-emerald-400 rounded-2xl hover:bg-emerald-600/30 transition-all border border-emerald-600/20 font-black text-xs uppercase tracking-widest flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                title={!patient.email ? 'Cadastre um e-mail para enviar o convite' : 'Enviar convite de acesso ao paciente'}
              >
                {invitingAccess ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}
                {patient.email ? 'Enviar convite' : 'Sem e-mail'}
              </button>
            )}
          </ProGuard>
