    ...
    const fetchFinancialData = async () => {
      ...
      try {
        ...
        const { data: appts, error } = await supabase
          .from('agendamentos')
          .select('id, status, data_servico, valor')
        ...
        const totalEarnings = appts?.reduce((acc, curr) => acc + (Number(curr.valor) || 0), 0) || 0;
        
        // Filter for current month
        const now = new Date();
        const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
        const monthlyAppts = appts?.filter(a => new Date(a.data_servico) >= firstDayOfMonth) || [];
        const monthlyEarnings = monthlyAppts.reduce((acc, curr) => acc + (Number(curr.valor) || 0), 0);
        ...
        const forecast = pendingAppts?.reduce((acc, curr) => acc + (Number(curr.valor) || 0), 0) || 0;
