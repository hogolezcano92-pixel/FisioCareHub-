...
const DEFAULT_IMAGE = 'https://images.unsplash.com/photo-1576091160550-2173dba999ef?auto=format&fit=crop&q=80&w=1200';
const FALLBACK_UPDATES: ClinicalUpdate[] = [
  ...
  },
  {
    ...
    source_type: 'Sistema',
    category: 'Evidência',
    image_url: 'https://images.unsplash.com/photo-1530026405186-ed1f139313f8?auto=format&fit=crop&q=80&w=1200',
  },
];
...
export default function ClinicalUpdatesCarousel({ className }: { className?: string }) {
  ...
  const categories = useMemo(() => {
    const values = updates.map((item) => item.category || 'Geral');
    return ['Todos', ...Array.from(new Set(values)).slice(0, 7)];
  }, [updates]);

  const visibleUpdates = useMemo(() => {
    if (selectedCategory === 'Todos') return updates;
    return updates.filter((item) => (item.category || 'Geral') === selectedCategory);
            ...
            >
              <div className="flex items-start gap-4">
                <div className="h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-slate-800">
                  <img
                    src={featuredUpdate.image_url || DEFAULT_IMAGE}
          ...
          {loading ? (
            <div className="flex gap-4 overflow-hidden">
              {Array.from({ length: 3 }).map((_, index) => (
                <div key={index} className="h-[330px] min-w-[245px] rounded-[1.75rem] border border-white/10 bg-white/5 animate-pulse" />
              ))}
            </div>
          ) : (
            <div
            ...
            >
              {visibleUpdates.map((item, index) => (
                <article
                  ...
                  className={cn(
                    'group relative flex h-[330px] w-[245px] shrink-0 snap-center flex-col overflow-hidden rounded-[1.75rem] border bg-slate-950/70 shadow-xl transition-all duration-300 sm:w-[285px]',
                    activeIndex === index
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col justify-between p-4">
                    ...
                    <button
                      ...
                      className="mt-4 inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-violet-600 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-blue-950/20 transition-all hover:scale-[1.02] disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      Ler fonte <ExternalLink size={14} />
                    </button>
                  </div>
                </article>
              ))}
