...
export default function DailyJournal() {
  ...
  const highlightedRegistro = searchParams.get('registro') || searchParams.get('entry') || searchParams.get('ref');
  ...
  useEffect(() => {
    if (!highlightedRegistro) return;

    setActiveTab('journal');

    const timer = window.setTimeout(() => {
      const target = document.getElementById(`journal-entry-${highlightedRegistro}`)
        || document.querySelector(`[data-journal-ref="${highlightedRegistro}"]`);
  ...
  }, [highlightedRegistro, history.length]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const entryData = {
        // registros_paciente.paciente_id tem FK para perfis.id.
        // Por isso o diário deve continuar salvando com user.id.
        // O vínculo com o fisioterapeuta fica em fisioterapeuta_id
        // e a área do fisioterapeuta busca também por pacientes.perfil_id.
        paciente_id: user?.id,
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20 mobile-bottom-content-safe">
                 ...
                 {history.length === 0 ? (
                   ...
                   history.map((entry) => (
                     <div
                       ...
                       data-journal-ref={entry.data_registro}
                       className={cn(
                         "p-5 bg-white/5 border border-white/5 rounded-2xl hover:border-white/10 transition-all group scroll-mt-28",
                         highlightedRegistro && (highlightedRegistro === entry.id || highlightedRegistro === entry.data_registro)
                           ? "border-sky-400/80 bg-sky-500/10 shadow-2xl shadow-sky-500/10"
                           : ""
                       )}
                     >
