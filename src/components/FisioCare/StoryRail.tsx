...
export default function StoryRail({ title = 'FisioStories', subtitle = 'Dicas, bastidores e agenda dos fisioterapeutas', physioId, className = '', compact = false }: StoryRailProps) {
  ...
  return (
    <section className={`relative overflow-hidden border border-white/10 bg-white/[0.04] shadow-2xl shadow-sky-950/10 backdrop-blur-xl ${compact ? 'rounded-[1.75rem] p-3 sm:p-4' : 'rounded-[2rem] p-4'} ${className}`}>
      <div className="absolute -right-20 -top-20 h-44 w-44 rounded-full bg-sky-500/10 blur-3xl" />
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-sky-400/20 bg-sky-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-sky-300">
      ...
      {loading ? (
        <div className="flex items-center gap-3 rounded-2xl bg-white/5 p-4 text-sm font-bold text-slate-400">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {groups.map((group, index) => {
            ...
            return (
              <button key={group.physio_id} onClick={() => setViewerIndex(index)} className={`${compact ? 'min-w-[74px]' : 'min-w-[86px]'} group flex flex-col items-center gap-2`}>
                <div className="rounded-full bg-gradient-to-tr from-sky-400 via-violet-500 to-fuchsia-500 p-[3px] shadow-lg shadow-sky-950/30">
                  <div className={`${compact ? 'h-14 w-14' : 'h-16 w-16'} grid place-items-center overflow-hidden rounded-full bg-slate-950 ring-4 ring-slate-950`}>
                ...
                <span className="line-clamp-2 max-w-[86px] text-center text-[11px] font-black leading-tight text-white group-hover:text-sky-300">{group.physio?.nome_completo || 'Fisioterapeuta'}</span>
                <span className="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-slate-500"><Camera size={10} /> {group.stories.length}</span>
              </button>
            );
          })}
        </div>
