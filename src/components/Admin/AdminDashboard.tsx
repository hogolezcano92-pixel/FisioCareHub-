        ...
        >
          ...
          <div className="space-y-2">
             ...
             <div className="flex items-center gap-1.5 text-emerald-600">
                ...
                <span className="text-xs text-slate-600 font-semibold">vs mês anterior</span>
             </div>
          </div>
        </motion.div>
            ...
            {(recentActivities || []).map((activity) => (
              <div key={activity.id} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-200 group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-600 font-medium truncate">USER: {(activity.usuario_id ?? '').split('-')[0]}</p>
                </div>
              </div>
            ))}
