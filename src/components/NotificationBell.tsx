...
export default function NotificationBell() {
  ...
  const getMarketingCtaLabel = (notification: any) => {
    const label = [notification?.metadata?.cta_label, notification?.metadata?.ctaLabel, notification?.cta_label, notification?.metadata?.button_text, notification?.metadata?.buttonText].find((v) => typeof v === 'string' && v.trim());
    return label?.trim() || 'Saiba mais';
  };

  const openNotification = async (notification: any) => {
    if (isMarketingNotification(notification)) { setSelectedMarketingNotification(notification); setIsOpen(false); return; }
    const link = getNotificationLink(notification);
    if (!link) return;
    await markAsRead(notification.id); setIsOpen(false);
    if (/^https?:\/\//i.test(link)) window.location.href = link; else navigate(link.startsWith('/') ? link : `/${link}`);
  };

  const openMarketingDestination = async () => {
    if (!selectedMarketingNotification) return;
    const notification = selectedMarketingNotification; const link = getNotificationLink(notification);
    await markAsRead(notification.id); setSelectedMarketingNotification(null);
    if (!link) return;
    if (/^https?:\/\//i.test(link)) window.location.href = link; else navigate(link.startsWith('/') ? link : `/${link}`);
  };
  ...
  const handleAction = async (notification: any, approved: boolean) => {
    ...
    try {
      ...
      await markAsRead(notification.id); import('sonner').then(({ toast }) => toast.success(approved ? 'Agendamento confirmado!' : 'Agendamento recusado. Solicitando estorno...'));
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <style>{`
        ...
        html:not(.dark) .fch-marketing-modal,html.light .fch-marketing-modal,body.light .fch-marketing-modal{background:#ffffff!important;color:#0f172a!important;border-color:rgba(124,58,237,.18)!important;box-shadow:0 35px 100px -25px rgba(15,23,42,.35)!important}
        html:not(.dark) .fch-marketing-modal .fch-marketing-title,html.light .fch-marketing-modal .fch-marketing-title,body.light .fch-marketing-modal .fch-marketing-title{color:#0f172a!important}
        html:not(.dark) .fch-marketing-modal .fch-marketing-message,html.light .fch-marketing-modal .fch-marketing-message,body.light .fch-marketing-modal .fch-marketing-message{color:#334155!important}
        html:not(.dark) .fch-marketing-modal .fch-marketing-close,html.light .fch-marketing-modal .fch-marketing-close,body.light .fch-marketing-modal .fch-marketing-close{background:#f8fafc!important;border-color:#e2e8f0!important;color:#475569!important}
        html:not(.dark) .fch-marketing-modal .fch-marketing-cta,html.light .fch-marketing-modal .fch-marketing-cta,body.light .fch-marketing-modal .fch-marketing-cta{color:#ffffff!important;background:linear-gradient(135deg,#2563eb,#7c3aed)!important;box-shadow:0 14px 30px -12px rgba(37,99,235,.45)!important}
      `}</style>
      ...
      <AnimatePresence>
        {isOpen && <motion.div initial={{ opacity: 0, y: 12, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: .96 }} transition={{ duration: .18, ease: 'easeOut' }} className="fch-notification-popover fixed left-4 right-4 top-[5.6rem] z-[9999] overflow-hidden rounded-[1.8rem] border border-white/10 shadow-[0_30px_90px_-28px_rgba(0,0,0,.85)] backdrop-blur-2xl sm:absolute sm:left-auto sm:right-0 sm:top-auto sm:mt-3 sm:w-[24rem]">
          <div className="fch-notification-head relative overflow-hidden border-b border-white/10 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 p-4"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_0%,rgba(59,130,246,.20),transparent_35%),radial-gradient(circle_at_88%_0%,rgba(124,58,237,.18),transparent_32%)]"/><div className="relative space-y-4"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-blue-500/12 text-blue-300 ring-1 ring-blue-300/20"><Sparkles size={18}/></div><div><h4 className="fch-light-title text-base font-black tracking-tight text-white">Notificações</h4><p className="fch-light-muted text-[11px] font-semibold text-slate-400">{unreadCount > 0 ? `${unreadCount} nova${unreadCount > 1 ? 's' : ''} para revisar` : notifications.length > 0 ? `${notifications.length} no histórico` : 'Tudo em dia por aqui'}</p></div></div>{unreadCount > 0&&<button type="button" onClick={markAllAsRead} className="inline-flex items-center gap-1.5 rounded-full border border-blue-400/20 bg-blue-500/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.16em] text-blue-300"><CheckCheck size={13}/>Lidas</button>}</div><div className="grid grid-cols-2 gap-2 rounded-2xl bg-white/[.04] p-1 ring-1 ring-white/10"><button type="button" onClick={()=>{setFilter('all');setShowAll(false)}} className={cn('rounded-xl px-3 py-2 text-[11px] font-black uppercase tracking-[.16em]',filter==='all'?'bg-gradient-to-r from-blue-600 to-violet-600 text-white':'text-slate-400')}>Todas</button><button type="button" onClick={()=>{setFilter('unread');setShowAll(false)}} className={cn('rounded-xl px-3 py-2 text-[11px] font-black uppercase tracking-[.16em]',filter==='unread'?'bg-gradient-to-r from-blue-600 to-violet-600 text-white':'text-slate-400')}>Não lidas ({unreadCount})</button></div></div></div>
          <div className="fch-notification-list max-h-[min(62vh,440px)] overflow-y-auto bg-slate-950/92">{filteredNotifications.length===0?<div className="p-10 text-center"><div className="mx-auto mb-4 grid h-16 w-16 place-items-center rounded-full bg-white/[.04] text-slate-500 ring-1 ring-white/10"><Bell size={30}/></div><p className="text-sm font-black text-slate-300">{filter==='unread'?'Sem notificações não lidas':'Nenhuma notificação por enquanto'}</p></div>:<div className="divide-y divide-white/[.06]">{visibleNotifications.map((n)=>{const tone=getNotificationTone(n.tipo);const isUnread=!n.lida;const notificationLink=getNotificationLink(n);const marketing=isMarketingNotification(n);return <div key={n.id} onClick={()=>openNotification(n)} className={cn('fch-notification-item group relative cursor-pointer overflow-hidden p-4 transition-all hover:bg-white/[.04]',isUnread&&'fch-notification-unread bg-blue-500/[.055]')}><div className={cn('pointer-events-none absolute inset-y-0 left-0 w-1 bg-gradient-to-b opacity-0 transition-opacity',tone.accent,isUnread&&'opacity-100')}/><div className="flex gap-3"><div className={cn('mt-0.5 grid h-10 w-10 shrink-0 place-items-center rounded-2xl',tone.iconWrap)}>{tone.icon}</div><div className="min-w-0 flex-1 space-y-2"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="mb-1 flex items-center gap-2"><span className="rounded-full bg-white/[.06] px-2 py-0.5 text-[9px] font-black uppercase tracking-[.16em] text-slate-400 ring-1 ring-white/10">{tone.label}</span>{isUnread&&<span className="h-2 w-2 rounded-full bg-blue-400"/>}</div><p className={cn('fch-light-title truncate text-sm font-black tracking-tight',isUnread?'text-white':'text-slate-300')}>{n.titulo||'Nova notificação'}</p></div><span className="inline-flex shrink-0 items-center gap-1 text-[10px] font-bold text-slate-500"><Clock size={11}/>{formatNotificationTime(n.created_at)}</span></div><p className="fch-light-muted line-clamp-3 whitespace-pre-wrap text-[12px] font-medium leading-relaxed text-slate-400">{n.mensagem||'Abra para ver mais detalhes.'}</p>{n.tipo==='appointment_request'&&isUnread&&<div className="grid grid-cols-2 gap-2 pt-1"><button type="button" onClick={(e)=>{e.stopPropagation();handleAction(n,true)}} className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 px-3 py-2 text-[10px] font-black uppercase tracking-[.12em] text-white"><Check size={13}/>Confirmar</button><button type="button" onClick={(e)=>{e.stopPropagation();handleAction(n,false)}} className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-[10px] font-black uppercase tracking-[.12em] text-slate-300"><X size={13}/>Recusar</button></div>}<div className="flex items-center justify-between gap-2 pt-1">{notificationLink?<span className="text-[10px] font-black uppercase tracking-[.18em] text-blue-300">{marketing?'Abrir campanha':'Ver detalhes'}</span>:<span className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-600">Aviso interno</span>}{isUnread&&<button type="button" onClick={(e)=>{e.stopPropagation();markAsRead(n.id)}} className="rounded-full border border-blue-400/15 bg-blue-500/10 px-2.5 py-1 text-[9px] font-black uppercase tracking-[.14em] text-blue-300 sm:opacity-0 sm:group-hover:opacity-100">Marcar lida</button>}</div></div></div></div>})}</div>}</div>
      ...
      <AnimatePresence>
        {selectedMarketingNotification && (() => {
          ...
          return <motion.div className="fixed inset-0 z-[10000] flex items-start justify-center overflow-y-auto bg-slate-950/70 px-4 py-6 sm:py-10 md:py-14 backdrop-blur-md" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onMouseDown={(e)=>{if(e.target===e.currentTarget)setSelectedMarketingNotification(null)}}>
            <motion.div initial={{opacity:0,y:10,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:10,scale:.98}} transition={{duration:.18}} className="fch-marketing-modal my-auto w-full max-w-lg max-h-[calc(100vh-3rem)] overflow-y-auto rounded-[1.75rem] border border-white/10 bg-slate-950 text-white shadow-[0_35px_100px_-25px_rgba(0,0,0,.85)] sm:max-h-[calc(100vh-5rem)]">
              {image&&<img src={image} alt="" className="h-36 w-full object-cover sm:h-44" onError={(e)=>{e.currentTarget.style.display='none'}}/>}
              <div className="p-5 sm:p-6"><div className="mb-5 flex items-start justify-between gap-4"><div className="flex items-center gap-3"><div className={cn('grid h-11 w-11 place-items-center rounded-2xl',tone.iconWrap)}>{tone.icon}</div><div><span className="text-[10px] font-black uppercase tracking-[.18em] text-violet-300">Marketing</span><p className="mt-1 text-xs font-semibold text-slate-500">FisioCareHub</p></div></div><button type="button" aria-label="Fechar" onClick={()=>setSelectedMarketingNotification(null)} className="fch-marketing-close grid h-9 w-9 place-items-center rounded-full border border-white/10 bg-white/[.04] text-slate-400 hover:text-white"><X size={17}/></button></div><h3 className="fch-marketing-title text-xl font-black tracking-tight text-white sm:text-2xl">{notification.titulo||'Nova campanha'}</h3><p className="fch-marketing-message mt-4 whitespace-pre-wrap text-sm font-medium leading-7 text-slate-300">{notification.mensagem||''}</p>{link&&<button type="button" onClick={openMarketingDestination} className="fch-marketing-cta mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-violet-600 px-5 py-3.5 text-xs font-black uppercase tracking-[.15em] text-white shadow-lg shadow-blue-950/30 transition hover:brightness-110"><ExternalLink size={15}/>{ctaLabel}</button>}{!link&&<button type="button" onClick={()=>setSelectedMarketingNotification(null)} className="mt-6 w-full rounded-2xl border border-white/10 bg-white/[.04] px-5 py-3.5 text-xs font-black uppercase tracking-[.15em] text-slate-300">Fechar</button>}</div>
            </motion.div>
          </motion.div>;
        })()}
      </AnimatePresence>
