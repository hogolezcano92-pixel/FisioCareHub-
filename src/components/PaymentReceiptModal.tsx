...
const normalizeMethod = (method?: string | null) => {
  ...
  if (normalized.includes('CARD') || normalized.includes('CREDIT')) return 'Cartão de crédito';
...
const downloadPaymentReceiptPDF = async (receipt: PaymentReceiptData) => {
  ...
  const issuedAt = formatDateTimeBR(receipt.issuedAt || new Date().toISOString());
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.text('Este comprovante registra a confirmação do pagamento do agendamento informado.', 20, 72);
  ...
  doc.setTextColor(71, 85, 105);
  doc.setFontSize(8);
  doc.text('Observação: este comprovante não substitui nota fiscal quando aplicável.', 14, y + 12);
  doc.text('Documento gerado automaticamente pelo FisioCareHub com base nos registros de pagamento da plataforma.', 14, y + 18);
          ...
          >
            <div className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 p-6 text-white md:p-8">
              ...
              <div className="relative flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.28em] text-blue-200">FisioCareHub</p>
                    <h2 className="mt-1 text-2xl font-black tracking-tight">Comprovante de pagamento</h2>
                    <p className="mt-2 max-w-md text-sm font-medium leading-relaxed text-slate-300">
                      Pagamento confirmado com segurança e vinculado ao agendamento informado.
                    </p>
                  </div>
                </div>
            ...
            <div className="grid gap-4 p-5 md:grid-cols-3 md:p-8">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                ...
                <p className="mt-1 text-sm font-black text-slate-950 dark:text-white">{receipt.appointmentDate || '—'}</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                ...
                <p className="mt-1 text-sm font-black text-slate-950 dark:text-white">Processado pela plataforma</p>
              </div>
            </div>
            ...
            <div className="px-5 pb-5 md:px-8 md:pb-8">
              ...
              <div className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-xs font-semibold leading-relaxed text-blue-900 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-100">
                Este comprovante foi gerado com base no registro interno de pagamento. Ele não substitui nota fiscal quando aplicável.
              </div>
