...
const buildCredentialSvg = ({
  ...
  const middleCenterX = 480;
  const crefitoPillWidth = 328;
  const crefitoPillX = middleCenterX - crefitoPillWidth / 2;
  const proPillWidth = 186;
  const proPillX = middleCenterX - proPillWidth / 2;
  const qrPanelX = 704;
  const qrPanelWidth = 268;
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" width="1016" height="640" viewBox="0 0 1016 640">
      <defs>
        ...
        <clipPath id="qrClip"><rect x="761" y="238" width="154" height="154" rx="28"/></clipPath>
      ...
      <g clip-path="url(#cardClip)">
        ...
        <rect x="${crefitoPillX}" y="388" width="${crefitoPillWidth}" height="66" rx="33" fill="#ffffff" fill-opacity="0.10" stroke="#ffffff" stroke-opacity="0.16"/>
        <text x="${middleCenterX}" y="431" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="22" font-weight="900" letter-spacing="3" fill="#f8fafc">CREFITO: ${safeCrefito}</text>

        ${isPro ? `
        <rect x="${proPillX}" y="470" width="${proPillWidth}" height="48" rx="24" fill="#f59e0b" fill-opacity="0.20" stroke="#facc15" stroke-opacity="0.55"/>
        <text x="${middleCenterX}" y="501" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="19" font-weight="900" letter-spacing="3" fill="#fde68a">PLANO PRO</text>` : ''}

        <rect x="${qrPanelX}" y="196" width="${qrPanelWidth}" height="288" rx="46" fill="#ffffff" fill-opacity="0.12" stroke="#ffffff" stroke-opacity="0.18"/>
        <rect x="${qrX}" y="238" width="${qrSize}" height="${qrSize}" rx="28" fill="#ffffff"/>
        <image href="${qrDataUrl}" x="${qrX}" y="238" width="${qrSize}" height="${qrSize}" preserveAspectRatio="xMidYMid meet" clip-path="url(#qrClip)"/>
        <text x="${qrCenterX}" y="428" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="18" font-weight="900" letter-spacing="2" fill="#cbd5e1">${safeCode}</text>
        <text x="${qrCenterX}" y="458" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="14" font-weight="800" fill="#94a3b8">Validar perfil</text>
          ...
          <div className="relative flex h-full flex-col justify-between gap-2">
            ...
            <div className="grid min-h-0 flex-1 grid-cols-[58px_minmax(0,1fr)_58px] items-center gap-2 sm:grid-cols-[112px_minmax(0,1fr)_112px] sm:gap-5">
              ...
              <div className="min-w-0 space-y-1.5 sm:space-y-3">
                ...
                <div className="flex w-fit max-w-full flex-col items-center gap-1 sm:gap-2">
                  <span className="inline-flex max-w-full items-center justify-center rounded-full border border-white/10 bg-white/10 px-2 py-0.5 text-center text-[7px] font-black uppercase tracking-widest text-white/90 sm:px-4 sm:py-1.5 sm:text-[10px]">
                    CREFITO: {crefito}
                  </span>
                  {isPro && (
                    <span className="inline-flex items-center justify-center rounded-full border border-amber-300/30 bg-amber-400/15 px-2 py-0.5 text-center text-[7px] font-black uppercase tracking-widest text-amber-200 sm:px-4 sm:py-1.5 sm:text-[10px]">
                      Plano Pro
                    </span>
                  )}
                </div>
