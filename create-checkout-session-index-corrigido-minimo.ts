...
serve(async (req) => {
  ...
  try {
    ...
    if (finalType === "subscription" || plan === "pro") {
      ...
      metadata.appointmentId = String(appointment_id)
      metadata.agendamento_id = String(appointment_id)
      metadata.agendamentoId = String(appointment_id)
    } else if (
      ...
      metadata.payment_area = "health_library"
